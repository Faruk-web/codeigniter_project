...
class RestController extends \CI_Controller
{
     ...
     * @param $config_file
     */
    protected function get_local_config($config_file)
    {
    ...
    protected function _detect_api_key()
    {
        ...
        $instance_name_variable = $this->config->item('rest_instance_name');

        // Work out the name of the SERVER entry based on config
        $key_name = 'HTTP_'.strtoupper(str_replace('-', '_', $api_key_variable));
        $instance_name = 'HTTP_'.strtoupper(str_replace('-', '_', $instance_name_variable));

        $this->rest->key = null;
        $this->rest->level = null;
        //TODO change user_id column to instance_id in DB
        $this->rest->user_id = null;
        $this->rest->ignore_limits = false;

        // Find the key from server or arguments
        $key = isset($this->_args[$api_key_variable]) ? $this->_args[$api_key_variable] : $this->input->server($key_name);
        $instance = isset($this->_args[$instance_name_variable]) ? $this->_args[$instance_name_variable] : $this->input->server($instance_name);

        if (($key && $instance)) {
            if (!($row = $this->rest->db
                ->where($this->config->item('rest_key_column'), $key)
                ->where($this->config->item('rest_instance_column'), $instance)
        ...
        $is_inserted = $this->rest->db
            ->insert(
                ...
                [
                    ...
                    'ip_address' => $this->input->ip_address(),
                    'time'       => date("Y-m-d H:i:s"),  // change applied for mysql datetime
    ...
    protected function _check_limit($controller_method)
    {
        ...
        if ($result === null) {
            ...
            $this->rest->db->insert($this->config->item('rest_limits_table'), [
                ...
                'api_key'      => $api_key,
                'count'        => 1,
                'hour_started' => date("Y-m-d H:i:s"), // change applied for mysql datetime
            ]);
        }

        // Been a time limit (or by default an hour) since they called
        elseif (strtotime($result->hour_started) < (time() - $time_limit)) {
            // change applied for mysql datetime

            // Reset the started period and count
            $this->rest->db
                ->where('uri', $limited_uri)
                ->where('api_key', $api_key)
                ->set('hour_started', date("Y-m-d H:i:s"))  // change applied for mysql datetime
                ->set('count', 1)
