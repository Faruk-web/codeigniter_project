    ...
    <div class="nav-tabs-custom">
        ...
        <div class="tab-content">
            ...
            if (isset($edit) || isset($create)) {
                if (isset($dataview)) {
                    $payment_type = $dataview->payment_type;
                    $deposit_date = $dataview->deposit_date;
                    $deposit_amount = $dataview->deposit_amount;
                } else {
                    $payment_type = set_value('payment_type');
                    $deposit_date = date('Y-m-d');
                    $deposit_amount = set_value('deposit_amount');
                }
            ?>
            <div class="tab-pane active">
                <div class="box-body">
                    ...
                    <div class="col-md-8">
                        <div class="form-group <?php echo (form_error('deposit_date')!='')?'has-error has-danger':''; ?>">
                            <label class="control-label col-sm-3 required">Date</label>
                            <div class="col-sm-9">
                                <input type="text" class="form-control datepicker" name="deposit_date" value="<?php echo $deposit_date; ?>" required>
                            </div>
                            <?php echo form_error('deposit_date'); ?>
                        </div>

                        <div class="form-group <?php echo (form_error('payment_type')!='')?'has-error has-danger':''; ?>">
                            <label class="control-label col-sm-3">Deposit Type</label>
                            <div class="col-sm-9">
                                <select class="form-control" name="payment_type">
                                <?php
                                foreach (['Cash', 'Cheque'] as $typ) {
                                    $sel = ($payment_type==$typ)?'selected':'';
                        ...
                        <div class="form-group <?php echo (form_error('deposit_amount')!='')?'has-error has-danger':''; ?>">
                            <label class="control-label col-sm-3 required">Amount</label>
                            <div class="col-sm-9">
                                <input type="number" step="1" min="0" class="form-control" name="deposit_amount" value="<?php echo $deposit_amount; ?>" required>
                            </div>
                    ...
                    <table class="table table-bordered table-hover dataTable">
                        ...
                        <tbody>
                            ...
                            <tr>
                                ...
                                <td><?php echo dateFormat($val->deposit_date)?></td>
                                <td><?php echo $val->payment_type?></td>
                                <td align="right"><?php echo numberFormat($val->deposit_amount, 1)?></td>
                                <td class="remove">
                                    <div class="dropdown">
