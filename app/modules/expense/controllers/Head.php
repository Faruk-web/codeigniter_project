...
class Head extends MX_Controller {
    ...
    function __construct()
    {
        ...
        $this->table = 'expense_head';
        $this->viewpath = 'expense/head_view';
        $this->path = 'expense/head';
    ...
    public function store()
    {
        ...
        if ($this->form_validation->run()==false) {
            ...
            if (is_numeric($insId)) {
                $this->session->set_flashdata('flash_msg', $this->alert->success('Expense Head Added Successfully.'));

                redirect($this->path.qString());
            } else {
                $this->session->set_flashdata('flash_msg', $this->alert->warning('Expense Head Adding Failed. '.$insId));
    ...
    public function update($id)
    {
        ...
        if ($this->form_validation->run()==false) {
            ...
            if (is_numeric($result)) {
                $this->session->set_flashdata('flash_msg', $this->alert->success('Expense Head Update Successfully.'));

                redirect($this->path.qString());
            } else {
                $this->session->set_flashdata('flash_msg', $this->alert->warning('Expense Head Update Failed. '.$result));
    ...
    public function delete($id)
    {
        ...
        if (is_numeric($result)) {
            $this->session->set_flashdata('flash_msg', $this->alert->success('Expense Head Delete Successfully.'));
        } else {
